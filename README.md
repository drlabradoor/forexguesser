...
разбор. Один бесплатный сигнал на пользователя (лимит выключается через `FREE_RUN_LIMIT_ENABLED`), дальше — переход
по диплинку в личку с уже подставленным сообщением.

Интерфейс — три таба: «Скриншот» (профиль, косметический баланс, загрузка и результат), «Сигналы» и «Торговля»
пока заглушки «Доступно в полной версии».
